...
public class OrdersService implements IOrdersService {
	...
	public OrdersDTO save(OrdersDTO dto) {
		...
		if (dto.getId() != null) {
			...
			if(ordersEntity.getDebt() <= 0L) {
				...
				// msgStr = "Don hang: " + ordersEntity.getCode() + " da thanh toan xong!<br> Tong so tien: " + dto.getTotal();
			} else {
				if(ordersEntity.getPaid() > 0L ) {
					ordersEntity.setStatus("partpaid");
					if(oldPaid - ordersEntity.getPaid() != 0) {
						strDescription += dateFormat.format(date) + " Thanh toan 1 phan: " + (ordersEntity.getPaid() - oldPaid) + " vnd.&#13;";
					}			
				} else {
			...
			// msgStr = "Quý khách có DH moi: <br>SP -  " + itemEntity.getName() + " <br>Dien tich - " + dto.getArea() + " <br>So tien - " + dto.getTotal();
		}
	...
	public boolean updatePaidStatus(Long orderid) {
		try {
			...
			ordersEntity.setStatus("paid");
			ordersEntity.setPaymentDate(date);
			ordersEntity.setDescription(ordersEntity.getDescription() + dateFormat.format(date) + " Thanh toan xong.&#13;");
	...
	@Override
	public List<OrdersDTO> findByFilter(Long customerId, String status, String content, String startDate, String endDate, String startPaymentDate, String endPaymentDate, Pageable pageable) {
		List<OrdersDTO> rs = new ArrayList<>();
		List<OrdersEntity> entites = orderRepository.findByFilter(customerId, status, content, startDate, endDate, startPaymentDate, endPaymentDate, pageable); 
		for (OrdersEntity entity : entites) {
			rs.add(orderConverter.toDto(entity));
		}
		return rs;
	...
	@Override
	public List<OrdersDTO> findByFilter2(Long customerId, String status, String content, String startDate, String endDate, String startPaymentDate, String endPaymentDate, int page, int limit) {
		List<OrdersDTO> rs = new ArrayList<>();
		List<OrdersEntity> entites = orderRepository.findByFilter2(customerId, status, content, startDate, endDate, startPaymentDate, endPaymentDate, (page-1)*limit, limit); 
		for (OrdersEntity entity : entites) {
			rs.add(orderConverter.toDto(entity));
		}
		return rs;
